function nowparameter=draw(folder,varargin)
ip = inputParser;
ip.addParameter('drawindex', [1,-1]);
ip.addParameter('startweektest', 51);
ip.addParameter('endweektest', 52);
ip.addParameter('path1', './ans/');
ip.addParameter('nowparameter', cell(1,5));
ip.parse(varargin{:});
inputs=ip.Results;
drawindex=inputs.drawindex;
startweektest=inputs.startweektest;
endweektest=inputs.endweektest;
path1=inputs.path1;
nowparameter=inputs.nowparameter;
...
for i=1:length(lossfile)
    ...
    writelist(fileindex,[path lossfile(i).name],indexs);
end
drawpic(drawindex,note);

end

%按照fileindex给数组赋值
function writelist(fileindex,filename,indexs)
global list;
global y;
load(filename);
...
%画图
function drawpic(drawindex,note)
global list;
